                                ...
                                <tbody>
                                    @foreach ($gameTypes as $gameType)
                                        <tr>
                                            ...
                                            <td class="text-center">{{ $gameType->code }}</td>
                                            <td class="text-center"><img src="{{ $gameType->getImgUrlAttribute() }}"
                                                    alt="" width="100px">
                                            </td>
                                            <td class="text-center">
                                                <span class="status-label" data-product-id="{{ $gameType->id }}">
                                                    {{ $gameType->status == 1 ? 'Active' : 'Inactive' }}
                                                </span>
                                            </td>
                                            <td class="text-center">
                                                <button class="btn btn-warning toggle-status-btn"
                                                    data-product-id="{{ $gameType->id }}"
                                                    data-status="{{ $gameType->status }}" style="width: 120px;">
                                                    {{ $gameType->status == 1 ? 'Deactivate' : 'Activate' }}
                                                </button>
    ...
    <script>
        document.querySelectorAll('.toggle-status-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                fetch('{{ route('admin.codegametypes.toggle-status', ':productId') }}'.replace(
                        ':productId',
                        productId), {
                        method: 'POST',
                        headers: {
